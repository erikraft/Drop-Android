...
public class SettingsFragment extends PreferenceFragmentCompat {
    ...
    public void onCreatePreferences(final Bundle savedInstanceState, final String rootKey) {
        ...
        if (diagnosticsEnabled != null && diagnosticsOpen != null) {
            diagnosticsOpen.setEnabled(diagnosticsEnabled.getSharedPreferences().getBoolean(diagnosticsEnabled.getKey(), false));
            diagnosticsEnabled.setOnPreferenceChangeListener((pref, newValue) -> {
                diagnosticsOpen.setEnabled((Boolean) newValue);
                return true;
            });
            diagnosticsOpen.setOnPreferenceClickListener(pref -> {
                if (!diagnosticsEnabled.getSharedPreferences().getBoolean(diagnosticsEnabled.getKey(), false)) {
                    Snackbar.make(requireView(), R.string.diagnostics_disabled, Snackbar.LENGTH_LONG).show();
                    return true;
                }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            floatingTextSelectionPref.setOnPreferenceChangeListener((pref, newValue) -> {
                getContext().getPackageManager().setComponentEnabledSetting(new ComponentName(getContext(), FloatingTextActivity.class), (Boolean) newValue ? PackageManager.COMPONENT_ENABLED_STATE_ENABLED : PackageManager.COMPONENT_ENABLED_STATE_DISABLED, PackageManager.DONT_KILL_APP);
                return true;
            });
        }
    ...
    private boolean isNotificationsCorrectlyEnabled() {
        ...
        return (Build.VERSION.SDK_INT < Build.VERSION_CODES.N || notificationManager.areNotificationsEnabled()) && (Build.VERSION.SDK_INT < Build.VERSION_CODES.O || notificationManager.getNotificationChannel("MYCHANNEL") == null || notificationManager.getNotificationChannel("MYCHANNEL").getImportance() != NotificationManager.IMPORTANCE_NONE);
    }
    ...
    private void updateDeviceNameSummary(final Preference pref) {
        if (prefs.contains(getString(R.string.pref_device_name))) pref.setSummary("Android " + prefs.getString(getString(R.string.pref_device_name), getString(R.string.app_name)));
        else pref.setSummary(R.string.pref_device_name_summary);
    }
    ...
    private Preference initUrlPreference(final @StringRes int pref, final String url) {
        ...
        if (preference != null) preference.setOnPreferenceClickListener(p -> { ShareUtils.openUrl(this, url); return true; });
        return preference;
    }
